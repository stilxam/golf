# EXPERIMENTAL: DOESNT WORK WELL YET
...
def _compute_loss(
    ...
    """Computes the mean squared error plus an L1 penalty on slope changes."""
    # MSE
    y_pred = jax.vmap(model)(x_data)
    mse_loss = jnp.mean(jnp.square(y_data - y_pred))

    # L1 penalty on slope changes
    ...
    l1_penalty = l1_lambda * jnp.sum(jnp.abs(jnp.diff(slopes)))
...
def _make_step(
    ...
    loss, grads = eqx.filter_value_and_grad(_compute_loss)(model, x_data, y_data, l1_lambda)
...
def fit(
    ...
    """Trains the PiecewiseModel to fit the provided data with L1 regularization."""
    ...
    for i in range(n_iterations):
        model, opt_state, loss = _make_step(model, optimizer, opt_state, x_arr, y_arr, l1_lambda)
